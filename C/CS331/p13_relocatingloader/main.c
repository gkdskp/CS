...
{
	FILE *objcode;
	int start_addr_in_mem, start_addr_in_prgrm, prgrm_len, line_len, addr = 0, addr_in_mem, i = 0;
	char record_code, prgrm_name[10], prgrm_line[80], *token, bitmask[30], binary_str[120], *opcode;
	...
	while(! feof(objcode))
	{
		...
		switch(record_code)
		{
			case 'H':
				fscanf(
					objcode, "^%[^^]^%x^%x", prgrm_name, &start_addr_in_prgrm, &prgrm_len
				);
				break;
			
			case 'T':
				fscanf(objcode, "^%x^%x^%[^^]^%s", &addr, &line_len, bitmask, prgrm_line);
				...
				while(token != NULL)
				{
					...
					addr_in_mem = (binary_str[i++] == '0')? addr_in_mem: addr_in_mem+start_addr_in_mem;
